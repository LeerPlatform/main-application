export default function SearchBar({ filterText, onFilterTextChange, onFilterTextEnter }) {
  function handleFilterTextChange(event) {
    onFilterTextChange(event.target.value)
  }

  function handleFilterTextKeyPress(event) {
    if (event.key === "Enter") {
      onFilterTextEnter()
    }
  }
  ...
  return (
    <div className="relative text-gray-600">
      <input
        ...
        onKeyPress={handleFilterTextKeyPress}
      />

      <button type="submit" className="absolute left-0 top-0 mt-3 ml-4 focus:outline-none text-gray-600" onClick={onFilterTextEnter}>
